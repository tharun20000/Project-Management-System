...
const StatusBadge = styled.div`
  ...
  color: ${({ status, theme }) => {
    ...
    if (status === "On Hold") return theme.yellow;
    if (status === "Cancelled") return theme.pink;
    return theme.primary;
  }};
  background-color: ${({ status, theme }) => {
    let color = theme.primary;
    if (status === "Completed") color = theme.green;
    if (status === "On Hold") color = theme.yellow;
    if (status === "Cancelled") color = theme.pink;
    return color + "15";
  }};
          ...
          <Bottom>
            ...
            <AvatarGroup>
              {item.members.map((member) => (
                <UserAvatar sx={{ marginRight: '-12px', width: '34px', height: '34px' }} user={member.id} />
              ))}
            </AvatarGroup>
          </Bottom>
        </Container>
